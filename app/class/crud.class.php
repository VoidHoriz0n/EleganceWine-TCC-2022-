 ...
 * Descrição: A Classe de CRUD genérico foi elaborada com o objetivo 
 * de auxlilar nas operações CRUDs em diversos SGBDS, possui 
 * funcionalidades para construir instruções de INSERT, UPDATE E 
 * DELETE onde as mesmas podem ser executadas nos principais SGBDs, 
 * exemplo SQL Server, MySQL e Firebird. Instruções SELECT são 
 * recebidas integralmente via parâmetro.                           
 *******************************************************************/  
class Crud{   
   ...
   */    
   private function buildDelete($arrayCondicao){   
        ...
        // Inicializa variáveis   $array = array['a']
        $sql = "";   
        $valCampos= "";   
   ...
   */  
   public function getSQLGeneric($sql, $arrayParams=null, $fetchAll=TRUE){  
      try {   
        ...
        if($fetchAll):   
          $dados = $stm->fetchAll(PDO::FETCH_OBJ);   // $r['nome']
        else:                                         // $r->nome;
   ...
   public function checaCliente()
    {
        return (isset($_SESSION['cliente']) && $_SESSION['cliente']);
    }
