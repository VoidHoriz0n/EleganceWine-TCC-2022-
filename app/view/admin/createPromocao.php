...
$stmt = $pdo->prepare("SELECT * FROM tb_produtos ORDER BY nome");
$stmt->execute();
$rs = $stmt->fetchAll();
...
<main>
  ...
  <div class="d-flex flex-column flex-shrink-0 p-3 bg-light" style="width: 280px;">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
      ...
      <li>
        <a href="createPromocao.php" class="nav-link active link-dark">
        <i class="fa-solid fa-folder-plus"></i>
          Cadastrar Promoção
        </a>
      </li>
...
<!--
  ...
  data_fim
  valor
  produtos_cod_produtos
-->
    
        <div class="container">
        <form method="POST">
            <label>Data de Inicio</label>
            <input type="date" class="form-control col-lg-6" id="data_inicio" name="data_inicio">
            <label>Data de Termino</label>
            <input type="date" class="form-control col-lg-6" id="data_fim" name="data_fim">
            <label>Valor</label>
            <input type="text" class="form-control col-lg-6" id="valor" name="valor">
            <br>
            <label>Produtos:</label>
    <!-- criando e preenchendo o 1º select com os dados da consulta -->
            <select name="produtos" id="produtos">
              <?php foreach ($rs as $r) { ?>
                 <option value="<?php echo $r['cod_produtos']; ?>"><?php echo $r['nome']; ?></option>
              <?php } ?>
            </select>
            <br><br>
...
<?php 
 ...
 if(isset($_POST['btCadastrar'])){
    ...
    $produtosselect       = isset($_POST['produtos'])     ? $_POST['produtos']   : null;

    if(empty($datainicio) 
    && empty($datafim)
    && empty($valor)
    && empty($produtosselect)
 ...
 // INSERT INTO `tb_promocoes`(`cod_promocao`, `data_inicio`, `data_fim`, `valor`, `produtos_cod_produtos`) VALUES ('[value-1]','[value-2]','[value-3]','[value-4]','[value-5]')
  $sql = "INSERT INTO tb_promocoes (data_inicio, data_fim, valor, produtos_cod_produtos) VALUES (:di, :df, :v, :fk)";
  /*
  INSERT INTO tb_promocoes (data_inicio, data_fim, valor, produtos_cod_produtos) 
  VALUES (:di,:df, :v,[value-5])*/

  $stmt = $pdo->prepare($sql);
  $stmt->bindParam(':di', $datainicio);
  $stmt->bindParam(':df', $datafim);
  $stmt->bindParam(':v', $valor);
  $stmt->bindParam(':fk', $produtosselect);
  $stmt->execute();
  $user = $stmt->fetch();
}
?>